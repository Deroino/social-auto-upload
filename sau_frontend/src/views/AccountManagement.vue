<template>
  <div class="account-management">
    ...
    <div class="account-tabs">
      ...
      <!-- 统一的账号列表容器 -->
      <div class="account-list-container" style="padding: 0 20px 20px 20px;">
        <div class="account-search">
          <el-input
            v-model="searchKeyword"
            placeholder="输入名称或平台搜索"
            prefix-icon="Search"
            clearable
          />
        ...
        <div v-else class="empty-data">
          <el-empty :description="`暂无${activeTab === 'all' ? '' : activeTab}账号数据`" />
        </div>
      </div>
    </div>
...
const filteredAccounts = computed(() => {
  const accounts = accountStore.accounts.filter(account =>
    ...
    account.platform.toLowerCase().includes(searchKeyword.value.toLowerCase())
  );
  ...
  return accounts.filter(account => account.platform === activeTab.value);
});
...
const connectSSE = (platform, name) => {
  ...
  // 获取平台类型编号
  // 从全局的 platformTypes 映射中创建反向映射 (平台名称 -> ID)
  const platformNameToIdMap = {};
  for (const [id, name] of Object.entries(platformTypes.value)) {
    platformNameToIdMap[name] = id.toString();
  }

  const type = platformNameToIdMap[platform] || '1'
    ...
    if (valid) {
      if (dialogType.value === 'add') {
        ...
        try {
          ...
          const platformNameToIdMap = {};
          for (const [id, name] of Object.entries(platformTypes.value)) {
            platformNameToIdMap[name] = parseInt(id);
          }
          const type = platformNameToIdMap[accountForm.platform] || 1;

...
def post_video_baijiahao(title,files,tags,account_file,category=TencentZoneTypes.LIFESTYLE.value,enableTimer=False,videos_per_day = 1, daily_times=None,start_days = 0):
    ...
    for index, file in enumerate(files):
        for cookie in account_file:
            ...
            print(f"标题：{title}")
            print(f"Hashtag：{tags}")
            app = BaiJiaHaoVideo(title, str(file), tags, publish_datetimes[index], cookie)
            asyncio.run(app.main(), debug=False)
...
def post_video_tiktok(title,files,tags,account_file,category=TencentZoneTypes.LIFESTYLE.value,enableTimer=False,videos_per_day = 1, daily_times=None,start_days = 0, thumbnail_path = ''):
    ...
    for index, file in enumerate(files):
        for cookie in account_file:
            ...
            print(f"标题：{title}")
            print(f"Hashtag：{tags}")
            app = TiktokVideo(title, str(file), tags, publish_datetimes[index], cookie, thumbnail_path)
            asyncio.run(app.main(), debug=False)
